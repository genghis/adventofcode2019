...
locationlist1.extend(startinggun)
locationlist2.extend(startinggun)

def mischief_managed(cursor,item):
	if item[0] == "R":
		response = [cursor[0],cursor[1] + int(item[1:])]
	elif item[0] == "L":
		response = [cursor[0],cursor[1] - int(item[1:])]
	elif item[0] == "U":
		response = [cursor[0] + int(item[1:]), cursor[1]]
	elif item[0] == "D":
		response = [cursor[0] - int(item[1:]), cursor[1]]
	return response[:]
	...
	if nextitem < len(locationlist1):
		if item[0] == locationlist1[nextitem][0]:
			if item[1] > locationlist1[nextitem][1]:
				...
				ridiculouslist1.extend([(item[0], y) for y in range(item[1], locationlist1[nextitem][1])])
		elif item[0] > locationlist1[nextitem][0]:
			ridiculouslist1.extend([(x,item[1]) for x in range(item[0], locationlist1[nextitem][0], -1)])
		else:
			ridiculouslist1.extend([(x,item[1]) for x in range(item[0], locationlist1[nextitem][0])])
	...
	if nextitem < len(locationlist2):
		if item[0] == locationlist2[nextitem][0]:
			if item[1] > locationlist2[nextitem][1]:
				...
				ridiculouslist2.extend([(item[0], y) for y in range(item[1], locationlist2[nextitem][1])])
		elif item[0] > locationlist2[nextitem][0]:
			ridiculouslist2.extend([(x,item[1]) for x in range(item[0], locationlist2[nextitem][0], -1)])
		else:
			ridiculouslist2.extend([(x,item[1]) for x in range(item[0], locationlist2[nextitem][0])])
...
for pair in intersections:
	if ridiculouslist1.index(pair) == 0:
		pass
	else:
		a = ridiculouslist1.index(pair)
		b = ridiculouslist2.index(pair)
		print(f"{pair}: {ridiculouslist1.index(pair)} + {ridiculouslist2.index(pair)}")
		kludge.append(ridiculouslist1.index(pair) + ridiculouslist2.index(pair))
